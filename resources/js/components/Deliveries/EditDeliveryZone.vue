<template>
    <div class="card-body">
        <form >
            ...
            <button type="submit" class="btn btn-primary mt-2" @click.prevent="updateDeliveryZone(zone.id)">Update</button>
        </form>
    </div>
...
export default {
    props:['zone','branches','countries','areas','deliveryzoneareas','routeUpdateDelivery'],
    components: { VueTimepicker },
    data(){
        return{
            marker: { lat: Number(this.zone.latitude), lng: Number(this.zone.longitude)} ,
            kilos:this.zone.radius / 1000,
            deliveryHours:[],
            selectedAreas:[],
            form: new Form({
                branch_id: this.zone.branch_id,
                country_id:this.zone.country_id,
                delivery_time:this.zone.delivery_time,
                delivery_fees:this.zone.delivery_fees,
                minimum_order_price:this.zone.minimum_order_price,
                estimated_preparation_time:this.zone.estimated_preparation_time,
                areas:[],
                area:this.zone.area,
    ...
    methods:{
        ...
        loadDeliveryHours(){
            var i =0;
            while(i <this.zone.delivery_hours.length)
            {
        ...
        updateDeliveryZone(id){
           this.form.put('/vendor/delivery/'+id).then(()=>{
                window.location.href="/vendor/delivery?update-success"
    ...
    created() {
        this.loadDeliveryHours();
      this.zone.delivery_hours.map((deliveryHour)=>{
          this.form.day.push(deliveryHour.day);
          this.form.from.push(deliveryHour.from);
          this.form.to.push(deliveryHour.to);
      })
        this.deliveryzoneareas.forEach((area)=>{
            this.selectedAreas.push(area.area);
            this.form.areas.push(area.area_id);
        })
    },
    name: "EditDeliveryZone"
}
</script>
